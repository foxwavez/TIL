...
class Rectangle {
  ...
  init(width: Int, height: Int, xPosition: Int, yPosition: Int, cornerRadius: Int) {
    ...
    // 생성자 안에서 생성자를 초기화 가능한 건 convenience init에서만 가능하다
  }
  
  convenience init(xPosition: Int) {
    // convenience init -> designated init -> overwrite
    self.init() // 기존의 정해진 값을 사용하도록(designated init) 생성자 초기화
    self.xPosition = xPosition
    // convenience init은 self.init()으로 초기화가 끝나고 난 후에 self.xPosition 이렇게 접근을 해서 값을 넣을 수 있다
  }
  
  convenience init(width: Int, height: Int, cornerRadius: Int) { // 값을 딱 3개만 받는다
