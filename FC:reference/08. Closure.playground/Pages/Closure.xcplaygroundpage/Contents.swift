...
print(1) // 전역함수 free function 어디에서나 만들 수 있는 함수
max(1, 2)
func globalFunction() { }
...
func outsideFunction() -> () -> () { // 함수에서 리턴 타입으로 함수를 담을 수 있다 함수이름(input) -> (ouput) -> (anotherFunction)
  var x = 0
  func nestedFunction() {
...
let nestedFunction = outsideFunction() // 함수도 변수에 담을 수 있다
nestedFunction()
nestedFunction()
nestedFunction()
 ...
 */
// 1번 문제 예.   "Swift" -> 5
let countString: (String) -> Int = { param in
    return param.count
}
countString("Swift")
...
performClosure(param: ) { // 소괄호를 앞으로 땡겨오고
  $0.count
}
...
multiClosureParams(closure1: { // closure2를 생략 하는 대신 closure2옆에 )소괄호를 뒤로 빼준다
  print("inline")
}) {
  print("trailing")
}
...
/*:
 ...
 ---
 */
func multiple2 (input: Int) -> Bool {
    let number = input % 2 == 0
    return number
}
multiple2(input: 10)


let multiple2: (Int) -> Bool = { param in
    let number = param % 2 == 0
    return number
}
multiple2(input: 10)
...
func multuply (param1: Int, parma2: Int) -> Int {
    let number = param1 * parma2
    return number
}
multuply(param1: 3, parma2: 6)

let multiply: (Int, Int) -> Int = { param1, param2 in
    let number = param1 * param2
    return number
}
multuply(param1: 4, parma2: 7)
...
// 3단계 - 문법 최적화
let stringCount2: (String) -> Int = { $0.count } //$0은 첫번째로 들어오는 파라미터
